...
class _MaplibreNewViewState extends State<MaplibreNewView>
  ...
  static const int _compassRotationSensorProtectedFastSamplesRequired = 3;
  ...
  static const double _compassRotationSensorProtectedCoastMaxOpposingDelta =
      2.5;
  static const double _compassRotationSensorProtectedCoastMaxStep = 2.5;
  static const int _compassRotationSensorLateralLockEscapeSamplesRequired = 3;
  static const double _compassRotationSensorLateralLockEscapeMinRawDelta = 32.0;
  static const double _compassRotationSensorLateralLockEscapeMinRateDegPerSec =
      220.0;
  static const double _compassRotationSensorLateralLockImmediateMinRawDelta =
      18.0;
  ...
  static const Duration _compassTiltSettleDuration = Duration(
    milliseconds: 260,
  );
  static const double _compassTiltSettleDelta = 4.0;
  static const Duration _compassTiltQuarantineDuration = Duration(
    milliseconds: 900,
  );
  ...
  static const double _compassRotationFollowLagBoostStep = 4.0;
  ...
  bool _isCompassTiltSettleActive(DateTime now) {
    return now.isBefore(_compassTiltSettleUntil);
  }

  bool _isCompassTiltQuarantineActive(DateTime now) {
  ...
  bool _isCompassLagBuildCandidate({
    ...
    if (tiltQuarantineActive || tiltLateralLockActive) {
      return false;
    }
  ...
  bool _isCompassTiltSettleJitter({
    required double rawDelta,
    required bool tiltSettleActive,
  }) {
    return tiltSettleActive &&
        rawDelta.abs() >= _compassMinCameraDelta &&
        rawDelta.abs() <= _compassTiltSettleDelta;
  }

  bool _isCompassTiltQuarantineJitter({
    required double rawDelta,
    required bool tiltQuarantineActive,
  }) {
    return tiltQuarantineActive &&
  ...
  String _compassTiltJitterReason({
    ...
    if (_isCompassTiltQuarantineJitter(
      rawDelta: rawDelta,
      tiltQuarantineActive: tiltQuarantineActive,
    )) {
  ...
  double? _followCompassSensorRotation({
    ...
    final protectedFastRotationCandidate =
        rawAbs >= _compassRotationSensorProtectedMinRawDelta &&
        rawDelta.sign == sensorDelta.sign &&
        protectedLagGrowing &&
    ...
    final protectedLagSeedCandidate =
        ...
        sensorAbs <= _compassRotationSensorProtectedFastMaxDelta &&
        turnRateDegPerSec.abs() <=
            _compassRotationSensorProtectedLagSeedMaxRateDegPerSec;
    ...
    final activeProtectedBurstRotationCandidate =
        tiltProtectionActive && protectedBurstRotationCandidate;
    final lateralLockImmediateRotationEscapeCandidate =
        tiltLateralLockActive &&
        rawDelta.sign == sensorDelta.sign &&
        rawAbs >= _compassRotationSensorLateralLockImmediateMinRawDelta &&
    ...
    final lateralLockRotationEscapeCandidate =
        ...
        (protectedBurstRotationCandidate &&
            ...
            rawAbs >= _compassRotationSensorLateralLockEscapeMinRawDelta &&
            turnRateDegPerSec.abs() >=
                _compassRotationSensorLateralLockEscapeMinRateDegPerSec);
    if (tiltLateralLockActive && !lateralLockRotationEscapeCandidate) {
      _compassSensorRotationLastRawAbs = rawAbs;
      return null;
    }
  ...
  double _followCompassProtectedRotationCoast({
    ...
    final maxStep = math.max(
      ...
      math.min(
        ...
        _compassRotationSensorProtectedCoastMaxStep,
      ),
    );
  ...
  double _stabilizeCompassTilt({
    ...
    if (shouldLogDecision) {
      DebugConsole.log(
        ...
        'lagBoostStep=${rotationLagBoostStep.toStringAsFixed(1)} '
  ...
  void _startCompassFollow() {
    ...
    DebugConsole.log(
      ...
      'tiltSettleMs=${_compassTiltSettleDuration.inMilliseconds} '
      ...
      'rotationFollowLagBoostDelta=$_compassRotationFollowLagBoostDelta '
      'rotationFollowLagBoostStep=$_compassRotationFollowLagBoostStep '
